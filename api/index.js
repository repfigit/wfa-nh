...
import { searchContracts, scrapeAllChildcareContracts, saveScrapedContracts, CHILDCARE_KEYWORDS } from '../src/scraper/nh-das-scraper.js';
import { detectStructuring, detectDuplicates, analyzeVendorConcentration, runFullFraudAnalysis, } from '../src/analyzer/fraud-detector.js';
import { scrapeFiscalYear, scrapeRecentYears, getAvailableFiscalYears, } from '../src/scraper/transparent-nh-scraper.js';
...
app.get('/api/payments', asyncHandler(async (req, res) => {
    ...
    const filters = {
        ...
        fiscal_year: req.query.fiscal_year ? parseInt(req.query.fiscal_year) : undefined,
...
app.get('/api/expenditures', asyncHandler(async (req, res) => {
    ...
    const fiscal_year = req.query.fiscal_year ? parseInt(req.query.fiscal_year) : undefined;
    let sqlQuery = `
...
app.get('/api/analyze/structuring', asyncHandler(async (req, res) => {
    await ensureInitialized();
    const threshold = req.query.threshold ? parseInt(req.query.threshold) : 10000;
    const windowDays = req.query.window_days ? parseInt(req.query.window_days) : 7;
